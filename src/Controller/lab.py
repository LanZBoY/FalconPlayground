...
class SSEAPI:
    ...
    async def on_get(self, req: Request, resp: Response):
        message_aio_connection = aioredis.ConnectionPool.from_url(
        ...
        async def emmitter():
            ...
            yield SSEvent(json={"Status": "OK"}, retry=10000, event="Start")
            while True:
                ...
                yield SSEvent(json=data, retry=10000, event=eventType)
    ...
    async def on_post(self, req: Request, resp: Response):
        reqData = await req.get_media()
        message_aio_connection = aioredis.ConnectionPool.from_url(
            REDIS_URL, db=13, decode_responses=True
        )
        redis_pool = aioredis.Redis.from_pool(message_aio_connection)
        await redis_pool.publish("WenTee", json.dumps(reqData))
